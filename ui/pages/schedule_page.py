...
class SchedulePage(ctk.CTkFrame):
    def __init__(self, master):
        ...
        if sys.platform != "win32":
            ctk.CTkLabel(
                self,
                text="Automatic startup is only supported on Windows.",
                text_color="#e5484d",
            ).pack(pady=40)
            return
        ...
        self.interval_var = ctk.StringVar(
            value=saved_interval if saved_interval in self.intervals else "1 Hour"
        )
        ...
        self.action_btn = ctk.CTkButton(
            self.status_card, text="", command=self.toggle_task, width=200
        )
    ...
    def on_interval_change(self, choice: str):
        ...
        self.error_label.configure(
            text="Schedule updated to " + choice, text_color="#10b981"
        )
    ...
    def refresh_status(self):
        ...
        if installed:
            self.status_label.configure(
                text="Background downloads are currently ENABLED."
            )
            self.action_btn.configure(
                text="Disable Automatic Downloads",
                fg_color="#e5484d",
                hover_color="#c13639",
            )
        else:
            self.status_label.configure(
                text="Background downloads are currently DISABLED."
            )
            self.action_btn.configure(
                text="Enable Automatic Downloads",
                fg_color=["#3a7ebf", "#1f538d"],
                hover_color=["#325882", "#14375e"],
            )

    def toggle_task(self):
        from core.startup import add_to_startup, is_in_startup, remove_from_startup
