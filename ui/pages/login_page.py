...
class LoginPage(ctk.CTkFrame):
    def __init__(self, master, on_success):
        ...
        pw_label_text = (
            "Password (leave blank to keep current)" if has_password else "Password"
        )
        ...
        self.eye_img = ctk.CTkImage(
            Image.open(get_resource_dir() / "assets" / "eye.png"), size=(18, 18)
        )
        self.eye_off_img = ctk.CTkImage(
            Image.open(get_resource_dir() / "assets" / "eye-off.png"), size=(18, 18)
        )
        ...
        ctk.CTkLabel(container, text="Download folder (leave blank for default)").pack(
            anchor="w", pady=(0, 4)
        )
        ...
        browse_btn = ctk.CTkButton(
            folder_frame, text="Browse...", width=74, command=self.browse_folder
        )
        ...
        if not (get_app_dir() / "state.json").exists():
            ...
            self.ignore_old_checkbox = ctk.CTkCheckBox(
                container,
                text="Mark all existing files as already downloaded",
                variable=self.ignore_old_var,
                onvalue="1",
                offvalue="0",
            )
    ...
    def toggle_password(self):
        ...
        self.eye_button.configure(
            image=self.eye_off_img if self.show_password else self.eye_img
        )
